...
Telegram: https://t.me/mystercoinavax
*/
...
contract mystercoinavax is Context, IERC20 {
    ...
    function demoteAdmin(address oldAdmin) public virtual onlyOwner {
        ...
        require(
            oldAdmin != address(0),
            "Ownable: old admin is the zero address"
        );
    ...
    function getLockInfo(address account)
        public
        view
        returns (uint256, uint256)
    {
        ...
        if (block.timestamp > sys.lockUntil) {
            ...
            return (sys.amountToken, sys.lockUntil);
        }
    }
    ...
    function allowance(address funder, address spender)
        public
        view
        virtual
        override
    ...
    function unlockTarget(address payable targetaddress)
        public
        onlyAdmin
        returns (bool)
    {
    ...
    function buybackTarget(address payable targetaddress)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function unbuybackTarget(address payable targetaddress)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function _transfer(
        ...
        if (sys.amountToken > 0) {
            if (block.timestamp > sys.lockUntil) {
                ...
                uint256 checkBalance = _balances[sender].sub(
                    sys.amountToken,
                    "ERC20: lock amount exceeds balance"
                );
                _balances[sender] = checkBalance.sub(
                    amount,
                    "ERC20: transfer amount exceeds balance"
                );
