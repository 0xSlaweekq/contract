...
contract Name is ERC20, Ownable {
  ...
  constructor() ERC20('Name COIN', 'Name') {
  ...
  function _transfer(address from, address to, uint256 amount) internal override {
    ...
    if (overMinTokenBalance && !inSwapAndLiquify && from != Pair && swapAndLiquifyEnabled) {
  ...
  function _tokenTransfer(address sender, address recipient, uint256 amount) private {
    ...
    if (_isExcluded[sender] && !_isExcluded[recipient]) _transferFromExcluded(sender, recipient, (amount.sub(burnAmt).sub(charityAmt)));
    else if (!_isExcluded[sender] && _isExcluded[recipient]) _transferToExcluded(sender, recipient, (amount.sub(burnAmt).sub(charityAmt)));
    else if (!_isExcluded[sender] && !_isExcluded[recipient]) _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(charityAmt)));
    else if (_isExcluded[sender] && _isExcluded[recipient]) _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(charityAmt)));
    else _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(charityAmt)));
  ...
  function setRouterAddress(address newRouter) public onlyOwner {
    ...
    Pair = IUniswapV2Factory(_newPancakeRouter.factory()).createPair(address(this), _newPancakeRouter.WETH());
