...
contract NODERewardManagement {
  ...
  constructor(uint256 _nodeMinPrice, uint256 _rewardPerValue, uint256 _claimTime) {
  ...
  function distributeRewards(uint256 gas, uint256 rewardValue) private returns (uint256, uint256, uint256) {
    distribution = true;
    uint256 numberOfnodeOwners = nodeOwners.keys.length;
    require(numberOfnodeOwners > 0, 'DISTRI REWARDS: NO NODE OWNERS');
    if (numberOfnodeOwners == 0) return (0, 0, lastIndexProcessed);
    ...
    while (gasUsed < gas && iterations < numberOfnodeOwners) {
      localLastIndex++;
      if (localLastIndex >= nodeOwners.keys.length) localLastIndex = 0;
  ...
  function createNode(address account, string memory nodeName, uint256 _nodeInitialValue) external onlySentry {
    ...
    totalNodesCreated++;
    if (autoDistri && !distribution) distributeRewards(gasForDistribution, rewardPerValue);
  }

  function isNameAvailable(address account, string memory nodeName) private view returns (bool) {
    NodeEntity[] memory nodes = _nodesOfUser[account];
    for (uint256 i; i < nodes.length; i++) {
      if (keccak256(bytes(nodes[i].name)) == keccak256(bytes(nodeName))) return false;
    }
    return true;
  }
  ...
  function binary_search(NodeEntity[] memory arr, uint256 low, uint256 high, uint256 x) private view returns (int256) {
    if (high >= low) {
      ...
      if (arr[mid].creationTime == x) return int256(mid);
      else if (arr[mid].creationTime > x) return binary_search(arr, low, mid - 1, x);
  ...
  function _addNodeValue(address account, uint256 _creationTime) external onlySentry returns (uint256) {
    ...
    if (rewardNode > 0) {
      if (prevMult >= tierLevel[5]) node.rewardMult += tierSlope[5];
      else if (prevMult >= tierLevel[4]) node.rewardMult += tierSlope[4];
      else if (prevMult >= tierLevel[3]) node.rewardMult += tierSlope[2];
      else if (prevMult >= tierLevel[2]) node.rewardMult += tierSlope[2];
      else if (prevMult >= tierLevel[1]) node.rewardMult += tierSlope[1];
      else node.rewardMult += tierSlope[0];
  ...
  function _addAllNodeValue(address account) external onlySentry returns (uint256) {
    ...
    for (uint256 i; i < nodesCount; i++) {
      ...
      if (_node.rewardAvailable > 0) {
        if (prevMult >= tierLevel[5]) _node.rewardMult += tierSlope[5];
        else if (prevMult >= tierLevel[4]) _node.rewardMult += tierSlope[4];
        else if (prevMult >= tierLevel[3]) _node.rewardMult += tierSlope[2];
        else if (prevMult >= tierLevel[2]) _node.rewardMult += tierSlope[2];
        else if (prevMult >= tierLevel[1]) _node.rewardMult += tierSlope[1];
        else _node.rewardMult += tierSlope[0];
...
contract LVT is ERC20, Ownable, PaymentSplitter {
  ...
  function _transfer(address from, address to, uint256 amount) internal override {
    ...
    if (from != owner() && to != uniswapV2Pair && to != address(uniswapV2Router) && to != address(this) && from != address(this)) {
      ...
      // anti whale
      if (
        to != futurUsePool && to != distributionPool && to != devPool && from != futurUsePool && from != distributionPool && from != devPool
      ) {
        uint256 totalSupply = totalSupply();
        uint256 walletBalance = balanceOf(address(to));
        require(amount.add(walletBalance) <= totalSupply.mul(maxTx).div(10000), 'STOP TRYING TO BECOME A WHALE. WE KNOW WHO YOU ARE.');
      }
    }
