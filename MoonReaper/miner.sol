...
contract Miner is MinerOwned, Pausable, ReentrancyGuard {
    ...
    function deposit(address partner) external payable whenStarted nonReentrant {
        ...
        else if (1 ether < msg.value && msg.value <= 5 ether) userInfo[_msgSender()].blockWithdraw = block.timestamp + 240 hours;
    ...
    function pendingReward(address account) public view returns (uint256) {
        return ((userInfo[account].stake * ((block.timestamp - userInfo[account].timestamp) / 86400) * userInfo[account].percentage) / 100);
    }
