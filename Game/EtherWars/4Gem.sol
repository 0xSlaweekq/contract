...
contract Gem is ERC721A, Ownable {
  uint256 constant MAX_SUPPLY_PLUS_ONE = 4996;
  ...
  function mint(uint256 _quantity) external payable {
    ...
    uint256 payForCount = 0 ether;
    uint256 freeMintCount = _freeMintedCount[msg.sender];
    if (freeMintCount <= 9) {
        _freeMintedCount[msg.sender] = _freeMintedCount[msg.sender] + _quantity;
        require(msg.value >= payForCount, "ETH sent not correct");
        _mint(msg.sender, _quantity);
    } else {
        require(freeMintCount > 9, "You used max free mints");
    }
...
contract CraftGem is ERC721A, Ownable {
  ...
  constructor(address _gemAddress) ERC721A("Craft Gem", "CraftGem") {
  ...
  function mutate(uint256[] calldata _gemIds) external payable {
    ...
    if (isGoingToBurn(_gemIds[2])) {
      ...
      payable(msg.sender).transfer(mutatePrice);
    } else {
      usedGem[_gemIds[2]] = true;
    }
