...
contract CyberBasketball is ERC721A, Ownable, Pausable {
  uint256 private constant MAX_SUPPLY = 500;
  uint256 private constant MAX_PER_WALLET = 10;
  uint256 public currentPrice = 0.01 ether;

  string private _name = 'Cyber Bassketball';
  string private _symbol = 'CyBall';

  string private baseURI;

  bool public freeMintOnly = false;
  ...
  function mint(uint256 _amount) external payable whenNotPaused {
    ...
    require(msg.value == _amount * currentPrice, 'ETH sent not correct');
    require(freeMintedCount[msg.sender] <= MAX_PER_WALLET, 'You used max free mints');

    _mint(msg.sender, _amount);
  }

  function setFreeMintStatus(bool value) external onlyOwner {
    freeMintOnly = value;
  }

  function changeMintPrice(uint256 _currentPrice) external onlyOwner {
    currentPrice = _currentPrice * 10 ** 15; // 1=0.001 10=0.01 100=0.1
  }
