...
contract Test is IERC20, Ownable {
  ...
  event UpdateFees(uint256 newLiqFee, uint256 newBrnFee, uint256 newMrkFee, uint256 newRefFee);
  ...
  /**
    ...
    * @param feesAndLimits_ uint256 parameters:
                    ...
                    [1] liquidityFee on transfers, updatable within limits after creation
                    [2] burnFee on transfers, only if _flags[0] is set
                    [3] marketingFee on transfers, only if _flags[1] is set
                    [4] referralFee on transfers, only if _flags[2] is set
                    [5] feeLimit of total fees, cannot be updated after creation
                    [6] liquidityThreshold, min amount of tokens to be swapped on transfers
                    [7] txLimit, max amount of transfer for non-privileged users
    * @param markAddr marketingAddress, only if _flags[1] is set
    */
  constructor(string memory name_, string memory symbol_, bool[4] memory flags_, uint256[8] memory feesAndLimits_, address markAddr) {
    ...
    require(feesAndLimits_[5] <= FLOAT_FACTOR / 2, 'Wrong limit');
    require(feesAndLimits_[1] + feesAndLimits_[2] + feesAndLimits_[3] + feesAndLimits_[4] <= feesAndLimits_[5], 'Fees too high');
    ...
    liqFee = feesAndLimits_[1];
    liquidityAddress = owner();
    liqStatus = flags_[3];
    feeLimit = feesAndLimits_[5];

    if (flags_[0]) brnFee = feesAndLimits_[2];

    if (flags_[1]) {
      mrkFee = feesAndLimits_[3];
      marketingAddress = markAddr;
    }
    if (flags_[2]) {
      refFee = feesAndLimits_[4];
      if (!flags_[1]) marketingAddress = markAddr;
    }

    require(
      feesAndLimits_[7] >= feesAndLimits_[0] / FLOAT_FACTOR, // txLimit >= totalSupply/10000
      'txLimit is too low'
    );
    require(
      feesAndLimits_[7] <= feesAndLimits_[0], // txLimit <= totalSupply
      'txLimit is too high'
    );
    require(
      feesAndLimits_[6] <= feesAndLimits_[7], // liqThreshold <= txLimit
      'liqThreshold is too high'
    );
    txLimit = feesAndLimits_[7];
    liqThreshold = feesAndLimits_[6];
    ...
    emit UpdateFees(feesAndLimits_[1], feesAndLimits_[2], feesAndLimits_[3], feesAndLimits_[4]);
    emit UpdateTxLimit(feesAndLimits_[7]);
    emit UpdateLiqThreshold(feesAndLimits_[6]);
  ...
  function setFee(uint256 newLiqFee, uint256 newBrnFee, uint256 newMrkFee, uint256 newRefFee) external onlyOwner {
    require(newLiqFee + newBrnFee + newMrkFee + newRefFee <= feeLimit, 'Fees too high');
    liqFee = newLiqFee;

    if (BRN_ENABLED) brnFee = newBrnFee;

    if (MRK_ENABLED) mrkFee = newMrkFee;

    if (REF_ENABLED) refFee = newRefFee;

    emit UpdateFees(newLiqFee, brnFee, mrkFee, refFee);
  }
  ...
  function _getFeeValues(uint256 amount, bool takeFee) private view returns (uint256 _liq, uint256 _brn, uint256 _mrk, uint256 _ref) {
    if (takeFee) {
  ...
  function _reflectFee(
    ...
    if (REF_ENABLED) {
      ...
      if (referrerAddress == address(0)) _takeFee(from, marketingAddress, mrk_ + ref, reflect, reflectBalance);
      else {
        _takeFee(from, marketingAddress, mrk_, reflect, reflectBalance);
        _takeFee(from, referrerAddress, ref, reflect, reflectBalance);
      }
    } else if (MRK_ENABLED) _takeFee(from, marketingAddress, mrk, reflect, reflectBalance);
  ...
  function _transfer(address from, address to, uint256 amount) private {
    ...
    (uint256 liq, uint256 brn, uint256 mrk, uint256 ref) = _getFeeValues(amount, takeFee);

    _updateBalances(from, to, amount, reflection, balance, liq + brn + mrk + ref);
    uint256 liqAmount = _reflectFee(from, reflection, balance, liq, brn, mrk, ref);
  ...
  function _updateBalances(address from, address to, uint256 amount, uint256 reflect, uint256 reflectBalance, uint256 fees) private {
    ...
    } else _reflections[to] += rTransferAmount;
