...
contract CyberBank is Ownable, ReentrancyGuard {
  ...
  function add(
    ...
    if (_rewardToken != IERC20(address(0))) _totalReward = _totalReward * 10 ** uint256(_rewardToken.decimals());
  ...
  function closePool(uint256 _pid) public validatePoolByPid(_pid) onlyOwner {
    ...
    if (block.timestamp < pool.endBlock) {
      ...
      if (pool.rewardToken != IERC20(address(0))) pool.rewardToken.safeTransfer(_msgSender(), rewardDiff);
      else transferMainnetToken(_msgSender(), rewardDiff);
    }
