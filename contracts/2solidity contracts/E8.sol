...
contract Energy8 is Context, Ownable, IERC20 {
  ...
  */
        ...
        if (_sellers[sender]) {
            ...
            if (!_whitelist[recipient] && !_whitelist[_msgSender()])
                _checkHodlPercent(
                    recipient,
                    amountWithFee,
                    "Recipient cannot hold this amount of tokens. Looks like he is already a whale!"
                );
            ...
            if (!_whitelist[sender] && !_whitelist[_msgSender()])
                _checkAndUpdatePeriod(
                    sender,
                    amount,
                    "You can not transfer this amount of tokens for the current period. Just relax and wait"
                );
        }
    ...
    function getAccountPeriodInfo(
        address account
    ) external view returns (uint256 startBalance, uint256 startTime, uint256 spent) {
