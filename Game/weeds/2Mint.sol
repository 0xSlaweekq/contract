...
interface IERC721 is IERC165 {
     ...
     */
    event ApprovalWeed(address indexed owner, address indexed operator, bool approved);
     ...
     * - If the caller is not `from`, it must be have been allowed to move this token by either {approve} or {setApprovalWeed}.
     ...
     * - If the caller is not `from`, it must be approved to move this token by either {approve} or {setApprovalWeed}.
     *
     * Emits a {Transfer} event.
     */
     ...
     *
     * Emits an {ApprovalWeed} event.
     */
    function setApprovalWeed(address operator, bool _approved) external;
     ...
     * See {setApprovalWeed}
     */
    function isApprovedWeed(address owner, address operator) external view returns (bool);
...
contract WeedsLab is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function setApprovalWeed(address operator, bool approved) public virtual override {
        _setApprovalWeed(_msgSender(), operator, approved);
    }

    function isApprovedWeed(address owner, address operator) public view virtual override returns (bool) {
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        if (_nftMetadata[tokenId]._nftType == 0) {
            mintedSols.push(tokenId);
        } 
