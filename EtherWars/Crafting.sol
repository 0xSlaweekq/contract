...
interface IERC721A {
  ...
  /**
   * The _amount of tokens minted must be more than zero.
   */
...
contract ERC721A is IERC721A {
  ...
  function approve(address to, uint256 tokenId) public override {
    ...
    if (_msgSenderERC721A() != owner)
      if (!isApprovedForAll(owner, _msgSenderERC721A())) revert ApprovalCallerNotOwnerNorApproved();
  ...
  /**
   * @dev Equivalent to `_safeMint(to, _amount, '')`.
   */
  function _safeMint(address to, uint256 _amount) internal {
    _safeMint(to, _amount, '');
  }

  /**
   * @dev Safely mints `_amount` tokens and transfers them to `to`.
   *
   * Requirements:
   *
  ...
  function _safeMint(address to, uint256 _amount, bytes memory _data) internal {
    ...
    _beforeTokenTransfers(address(0), to, startTokenId, _amount);

    // Overflows are incredibly unrealistic.
    // balance or numberMinted overflow if current value of either + _amount > 1.8e19 (2**64) - 1
    // updatedIndex overflows if _currentIndex + _amount > 1.2e77 (2**256) - 1
    unchecked {
      // Updates:
      // - `balance += _amount`.
      ...
      _packedAddressData[to] += _amount * ((1 << BITPOS_NUMBER_MINTED) | 1);

      // Updates:
      // - `address` to the owner.
      // - `startTimestamp` to the timestamp of minting.
      // - `burned` to `false`.
      // - `nextInitialized` to `_amount == 1`.
      _packedOwnerships[startTokenId] =
        _addressToUint256(to) |
        (block.timestamp << BITPOS_START_TIMESTAMP) |
        (_boolToUint256(_amount == 1) << BITPOS_NEXT_INITIALIZED);

      uint256 updatedIndex = startTokenId;
      uint256 end = updatedIndex + _amount;
    ...
    _afterTokenTransfers(address(0), to, startTokenId, _amount);
  }

  /**
   * @dev Mints `_amount` tokens and transfers them to `to`.
   *
   * Requirements:
   *
   * - `to` cannot be the zero address.
  ...
  function _transfer(address from, address to, uint256 tokenId) private {
    ...
    unchecked {
      ...
      _packedOwnerships[tokenId] = _addressToUint256(to) | (block.timestamp << BITPOS_START_TIMESTAMP) | BITMASK_NEXT_INITIALIZED;
...
contract Crystal is ERC721A, Ownable, Pausable {
  uint256 private constant MAX_SUPPLY = 4500;
  uint256 private constant MAX_PER_TRANSACTION = 3;

  string private baseURI;
  mapping(address => uint256) public freeMintedCount;

  constructor() ERC721A('Crystal Unrefined', 'CRLU') {
    _pause();
  }
...
contract CraftCrystal is ERC721A, Ownable {
  uint256 public craftPrice = 0.01 ether;
  ...
  mapping(uint256 => bool) public usedCrystal;

  Crystal private immutable crystalContract;
  ...
  function crafting(uint256[] calldata _crystalIds) external payable {
    require(_crystalIds.length == 3, 'Needs 3 crystal to crafting');
    require(msg.value == craftPrice, 'ETH sent not correct');

    require(_crystalIds[0] != _crystalIds[1], 'No duplicate crystal allowed');
    ...
    usedCrystal[_crystalIds[0]] = true;
    usedCrystal[_crystalIds[1]] = true;

    crystalContract.transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, _crystalIds[2]);
