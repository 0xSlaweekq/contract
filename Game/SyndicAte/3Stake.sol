...
contract Staking is Ownable, IERC721Receiver {
    ...
    ISyndicateGame nftContract;
    IERC20 token;

    uint256 private totalFarmed;

    struct UserInfo {
        uint256[] stakedWorkers;
        uint256[] stakedAgents;
        uint256[] stakedRobots;
    ...
    uint256[] private stakedWorkers;
    uint256[] private stakedAgents; // array of staked Agents TokenIDs
    uint256[] private stakedRobots;

    uint256 private workerReward = 100 * 10**18;
    uint256 private robotReward = 200 * 10**18;

    uint256 public stealPrice = 20 * 10**16;
    ...
    constructor(address _token, address _nftContract) {
        nftContract = ISyndicateGame(_nftContract);
        token = IERC20(_token);
    }
    ...
    function getNFTpending(uint256 tokenId) external view returns (uint256) {
        ...
        if (nft.nftType == 0) {
            return _pendingWorkersReward(tokenId);
        } else if (nft.nftType == 1) {
            return _pendingAgentsReward(tokenId);
        } else if (nft.nftType == 2) {
            return _pendingRobotsReward(tokenId);
        } else {
            return 0;
        }
    }
    ...
    function getNumOfStakedWorkers() public view returns (uint256) {
        return stakedWorkers.length;
    }

    function getNumOfStakedAgents() public view returns (uint256) {
        return stakedAgents.length;
    }

    function getNumOfStakedRobots() public view returns (uint256) {
        return stakedRobots.length;
    }
    ...
    function getStakedTokens(address owner) external view returns(uint256[] memory){
        ...
        uint256 length = user.stakedWorkers.length + user.stakedAgents.length + user.stakedRobots.length;
        uint256[] memory tokenIds = new uint256[](length);
        uint256 counter;
        for(uint256 i = 0; i < user.stakedWorkers.length; i ++) {
            tokenIds[counter] = user.stakedWorkers[i];
            counter ++;
        }
        for(uint256 i = 0; i < user.stakedAgents.length; i ++) {
            tokenIds[counter] = user.stakedAgents[i];
            counter ++;
        }
        for(uint256 i = 0; i < user.stakedRobots.length; i ++) {
            tokenIds[counter] = user.stakedRobots[i];
            counter ++;
        }
        return (tokenIds);
    }
    ...
    function harvestAll() external {
        ...
        for (uint256 i = 0; i < user.stakedWorkers.length; i ++) {
            _harvestNormal(user.stakedWorkers[i]);
        }
        for (uint256 i = 0; i < user.stakedAgents.length; i ++) {
            _harvestNormal(user.stakedAgents[i]);
        }
        for (uint256 i = 0; i < user.stakedRobots.length; i ++) {
            _harvestNormal(user.stakedRobots[i]);
        }
    }
    ...
    function stealReward(uint256 tokenId) external payable {
        UserInfo storage user = userInfo[_msgSender()]; 
        NFTInfo storage nft = nftInfo[tokenId];
        require(nft.nftType == 1, "Function is only for staked Agents");
    ...
    function _stake(uint256 tokenId) internal {
        ...
        if (nftType == 0) {
            user.stakedWorkers.push(tokenId);
            stakedWorkers.push(tokenId);
        } else if (nftType == 1) {
            user.stakedAgents.push(tokenId);
            stakedAgents.push(tokenId);
            _add(tokenId);
        } else if (nftType == 2) {
            user.stakedRobots.push(tokenId);
            stakedRobots.push(tokenId);
        } else {
    ...
    function _unstake(uint256 tokenId) internal {
        ...
        if (nft.nftType == 0) {
            for (uint256 i = 0; i < user.stakedWorkers.length; i++) {
                if (user.stakedWorkers[i] == tokenId) {
                    for (uint x = i; x < user.stakedWorkers.length - 1; x++) {
                        user.stakedWorkers[x] = user.stakedWorkers[x + 1];
                    }
                    user.stakedWorkers.pop();
                    found = true;
                }
            }
            for (uint256 i = 0; i < stakedWorkers.length; i++) {
                if (stakedWorkers[i] == tokenId) {
                    for (uint256 x = i; x < stakedWorkers.length - 1; x++) {
                        stakedWorkers[x] = stakedWorkers[x+1];
                    }
                    stakedWorkers.pop();
                }
            }
            ...
            for (uint256 i = 0; i < user.stakedAgents.length; i++) {
                if (user.stakedAgents[i] == tokenId) {
                    for (uint x = i; x < user.stakedAgents.length - 1; x++) {
                        user.stakedAgents[x] = user.stakedAgents[x + 1];
                    }
                    user.stakedAgents.pop();
                    found = true;
                }
            }    
            for (uint256 i = 0; i < stakedAgents.length; i++) {
                if (stakedAgents[i] == tokenId) {
                    for (uint256 x = i; x < stakedAgents.length - 1; x++) {
                        stakedAgents[x] = stakedAgents[x+1];
                    }
                    stakedAgents.pop();
                }
            }
            ...
            for (uint256 i = 0; i < user.stakedRobots.length; i++) {
                if (user.stakedRobots[i] == tokenId) {
                    for (uint x = i; x < user.stakedRobots.length - 1; x++) {
                        user.stakedRobots[x] = user.stakedRobots[x + 1];
                    }
                    user.stakedRobots.pop();
                    found = true;
                }
            }
            for (uint256 i = 0; i < stakedRobots.length; i++) {
                if (stakedRobots[i] == tokenId) {
                    for (uint256 x = i; x < stakedRobots.length - 1; x++) {
                        stakedRobots[x] = stakedRobots[x+1];
                    }
                    stakedRobots.pop();
                }
            } 
    ...
    function _harvestNormal(uint256 tokenId) internal {
        ...
        if (farmStartDate != 0 && farmStartDate <= block.timestamp) {
            ...
            if (nft.nftType == 0) {
                pendingReward_ = _pendingWorkersReward(tokenId);
                if (stakedAgents.length > 0 && userInfo[_msgSender()].stakedRobots.length == 0 && pendingReward_ < 300*10**18) {
                ...
                pendingReward_ = timeDiff * robotReward / DAY;
            } else {
                revert("Token metadata is unreachable");
            }
            nft.lastHarvest = block.timestamp;
    ...
    function _harvestUnstake(uint256 tokenId) internal {
        ...
        if (farmStartDate != 0 && farmStartDate <= block.timestamp) {
            ...
            if (nft.nftType == 0) {
                pendingReward_ = _pendingWorkersReward(tokenId);
                require(pendingReward_ >= 300*10**18, "300 tokens were not farmed yet");
                if (stakedAgents.length > 0) {
    ...
    function _pendingReward(address _address) internal view returns (uint256 pendingReward_) {
        ...
        if (user.stakedWorkers.length > 0) {
            for (uint256 i = 0; i < user.stakedWorkers.length; i ++) {
                pendingReward_ += _pendingWorkersReward(user.stakedWorkers[i]);
            }
        }
        if (user.stakedAgents.length > 0) {
            for (uint256 i = 0; i < user.stakedAgents.length; i++) {
                pendingReward_ += _pendingAgentsReward(user.stakedAgents[i]);
            }
        } 
        if (user.stakedRobots.length > 0) {
            for (uint256 i = 0; i < user.stakedRobots.length; i++) {
                pendingReward_ += _pendingRobotsReward(user.stakedRobots[i]);
            }
        }
    }

    function _pendingWorkersReward(uint256 tokenId) internal view returns (uint256) {
        if (farmStartDate == 0 || farmStartDate > block.timestamp) {
            return 0;
        }
        NFTInfo storage nft = nftInfo[tokenId];
        if (nft.owner != address(0)) {
            ...
            return timeDiff * workerReward / DAY - nft.amountStolen;
        } else {
            return 0;
        }
    }

    function _pendingAgentsReward(uint256 tokenId) internal view returns (uint256) {
        return dividendOf(tokenId);
    }

    function _pendingRobotsReward(uint256 tokenId) internal view returns (uint256) {
        if (farmStartDate == 0 || farmStartDate > block.timestamp) {
            return 0;
        }
        NFTInfo storage nft = nftInfo[tokenId];
        if (nft.owner != address(0)) {
            ...
            return timeDiff * robotReward / DAY;
        } else {
            return 0;
        }
    }
        ...
        if (stakedAgents.length > 0) {
            ...
            if (returned) {
                for (uint256 i = 0; i < _stolenNFTs.length; i++) {
                    ...
                    uint256 winId = stakedAgents[_luckyWinner];
    ...
    function _stealReward(UserInfo storage user) internal {
        uint256 _randomWorker = uint256(keccak256(abi.encodePacked(
                blockhash(block.number),
                tx.origin,
                block.timestamp + 20
            ))) % stakedWorkers.length;

        uint256 tokenId = stakedWorkers[_randomWorker];
        address owner = nftInfo[tokenId].owner;
        uint256 totalStolenReward;
        for (uint256 i = 0; i < userInfo[owner].stakedWorkers.length; i++) {
            uint256 stolenReward;
            tokenId = userInfo[owner].stakedWorkers[i];
            if (user.stakedRobots.length > 0) {
                stolenReward = _pendingWorkersReward(tokenId) * 5 / 10;
                totalStolenReward += stolenReward;
            } else {
                stolenReward = _pendingWorkersReward(tokenId) * 3 / 10;
                totalStolenReward += stolenReward;
            }
    ...
    function distributeDividends(uint256 amount) internal {
        require(stakedAgents.length > 0);

        magnifiedDividendPerShare = magnifiedDividendPerShare.add(
            (amount).mul(magnitude) / stakedAgents.length
        );
    }
