...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function setPoolInfo(
        ...
        if (_rewardForEachBlock > ZERO) pool.rewardForEachBlock = _rewardForEachBlock;
    ...
    function getMultiplier(uint256 _from, uint256 _to) public pure returns (uint256) {
        if (_to > _from) return _to.sub(_from);
    ...
    function deposit(uint256 _pid, uint256 _amount) external payable validatePoolByPid(_pid) {
        ...
        if (pool.lpToken == IERC20(address(0))) require(_amount == msg.value, "msg.value must be equals to amount!");

        UserInfo storage user = userInfo[_pid][msg.sender];
        harvest(_pid, msg.sender);
        if (pool.lpToken != IERC20(address(0))) pool.lpToken.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function emergencyStop(address payable _to) public onlyOwner {
        if (_to == address(0)) _to = payable(msg.sender);

        uint256 addrBalance = token.balanceOf(address(this));
        if (addrBalance > ZERO) token.safeTransfer(_to, addrBalance);
    ...
    function addRewardForPool(
        ...
        else addTokenPerBlock = _addTokenPerBlock.div(pool.endBlock.sub(block.timestamp));

        pool.rewardForEachBlock = pool.rewardForEachBlock.add(addTokenPerBlock);
        if (_withTokenTransfer) token.safeTransferFrom(msg.sender, address(this), _addTokenPerBlock);
