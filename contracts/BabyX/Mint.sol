...
contract Mint is ERC165, IERC721, IERC721Metadata, Ownable {
  ...
  function mint(uint256 _amount) external {
    ...
    for (uint256 i; i < _amount; i++) {
      ...
      if (_circulatingSupply == 3) _startFarming();
    }
  }
  ...
  function _getCurrentPrice() internal view returns (uint256) {
    if (block.timestamp <= priceChange[0].startTime + 3600) return priceChange[0].newPrice;
    else return gen0Price;
  }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    } else return string(abi.encodePacked(notRevealedURI, tokenId.toString())); // if do not reveal delete tokenId.toString()
  }
  ...
  function _mint(address to, uint256 tokenId) internal virtual {
    ...
    if (_nftMetadata[tokenId]._nftType == 1) mintedSilver.push(tokenId);
    else if (_nftMetadata[tokenId]._nftType == 2) mintedGold.push(tokenId);
    else mintedBronze.push(tokenId);

    bool stolen;
    if (_circulatingSupply > 3) stolen = _stealMint(tokenId);
