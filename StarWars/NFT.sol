...
contract ForceNFT is ERC165, IERC721, IERC721Metadata, Ownable {
  ...
  function mint(uint256 _amount) external payable {
    ...
    if (_circulatingSupply < 5000 && _circulatingSupply + _amount < 5000) {
      ...
      if (msg.value > _amount * gen0Price) payable(msg.sender).transfer(msg.value - _amount * gen0Price);
  ...
  function _mint(address to, uint256 tokenId) internal virtual {
    ...
    else if (tokenId > 5000 && tokenId <= 10000) _nftMetadata[tokenId].gen = 1;
    else if (tokenId > 10000) _nftMetadata[tokenId].gen = 2;
