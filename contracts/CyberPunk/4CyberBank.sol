...
contract CyberBank is Ownable, ReentrancyGuard {
    ...
    function add(
        ...
        // if (_rewardToken != IERC20(address(0))) _totalReward = _totalReward * 10 ** _rewardToken.decimals();
        if (_rewardToken != IERC20(address(0))) _totalReward = _totalReward * 10 ** 18;
        else _totalReward = _totalReward * 10 ** 18;
    ...
    function setPoolInfo(
        ...
        if (_startBlock > ZERO) {
            if (_endBlock > ZERO) require(_startBlock < _endBlock, "start block must less than end block!");
            else require(_startBlock < pool.endBlock, "start block must less than end block!");

            pool.startBlock = _startBlock;
        }
        if (_endBlock > ZERO) {
            if (_startBlock <= ZERO) require(pool.startBlock < _endBlock, "start block must less than end block!");
    ...
    function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        if (block.number < pool.startBlock) return;

        if (pool.lastRewardBlock >= pool.endBlock) return;

        if (pool.lastRewardBlock < pool.startBlock) pool.lastRewardBlock = pool.startBlock;
    ...
    function pendingReward(
        uint256 _pid,
        address _user
    ) public view validatePoolByPid(_pid) returns (uint256 tokenReward) {
        PoolInfo storage pool = poolInfo[_pid];
        if (_user == address(0)) _user = msg.sender;

        UserInfo storage user = userInfo[_pid][_user];
        uint256 accTokenPerShare = pool.accTokenPerShare;
        uint256 lpSupply = pool.amount;
        uint256 lastRewardBlock = pool.lastRewardBlock;
        if (lastRewardBlock < pool.startBlock) lastRewardBlock = pool.startBlock;
        ...
        if (
            ...
            if (block.number > pool.endBlock) multiplier = getMultiplier(lastRewardBlock, pool.endBlock);
            else multiplier = getMultiplier(lastRewardBlock, block.number);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) {
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(block.number >= pool.startBlock, "this pool has not started!");
        // if (pool.lpToken != IERC20(address(0))) _amount = _amount * 10 ** uint256(pool.lpToken.decimals());
        if (pool.lpToken != IERC20(address(0))) _amount = _amount * 10 ** 18;
        else if (pool.lpToken == IERC20(address(0))) _amount = _amount * 10 ** 18;
        ...
        if (pool.lpToken != IERC20(address(0)))
            ...
            //if pool is HT
        else transferMainnetToken(payable(msg.sender), _amount);

        emit Withdraw(msg.sender, _pid, _amount);
    }

    //transfer HT
    function transferMainnetToken(address _to, uint256 _amount) internal {
    ...
    function harvest(
        uint256 _pid,
        address _to
    ) public payable nonReentrant validatePoolByPid(_pid) returns (bool success) {
        ...
        if (user.amount != 0) {
            ...
            if (pending > ZERO) {
                success = true;
                if (pool.rewardToken == IERC20(address(0))) transferMainnetToken(payable(_to), pending);
                else safeTransferTokenFromThis(pool.rewardToken, _to, pending);
    ...
    function closePool(uint256 _pid) public validatePoolByPid(_pid) onlyOwner {
        ...
        if (block.timestamp < pool.endBlock) {
            ...
            if (pool.rewardToken != IERC20(address(0))) pool.rewardToken.safeTransfer(msg.sender, rewardDiff);
            else transferMainnetToken(msg.sender, rewardDiff);
        }
    ...
    function safeTransferTokenFromThis(IERC20 _token, address _to, uint256 _amount) internal {
        ...
        if (_amount > bal) _token.safeTransfer(_to, bal);
    ...
    function addRewardForPool(
        ...
        // if (pool.rewardToken != IERC20(address(0))) _addTotalTokens = _addTotalTokens * 10 ** uint256((pool.rewardToken.decimals()));
        if (pool.rewardToken != IERC20(address(0))) _addTotalTokens = _addTotalTokens * 10 ** 18;
        else _addTotalTokens = _addTotalTokens * 10 ** 18;

        uint256 addTokenPerBlock;
        if (block.number < pool.startBlock) addTokenPerBlock = _addTotalTokens.div(pool.endBlock.sub(pool.startBlock));
        else addTokenPerBlock = _addTotalTokens.div(pool.endBlock.sub(block.number));

        pool.rewardForEachBlock = pool.rewardForEachBlock.add(addTokenPerBlock);
        if (_withTokenTransfer) pool.rewardToken.safeTransferFrom(msg.sender, address(this), _addTotalTokens);
