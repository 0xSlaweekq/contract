...
abstract contract MinerBasic {
    ...
    event RehireMachines(
        ...
        uint256 _nInvestors,
        uint256 _referralGreens,
        uint256 _marketGreens,
        uint256 _GreensUsed
    );
    ...
    function calculateMarketingTax(uint256 amount) internal view returns (uint256) {
    ...
    function calculateFullTax(uint256 amount) internal view returns (uint256) {
        return SafeMath.div(SafeMath.mul(amount, devFeeVal + marketingFeeVal), 100);
    }
