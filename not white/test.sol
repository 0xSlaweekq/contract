...
contract BuybackBabyToken is IERC20Extended, Auth, BaseToken {
    ...
    function _transfer(
        ...
        // Buyback triggered on a sell when tokens added back to LP and ETH extracted by paperhanded bitches.
        if (buyBackEnabled && to == uniswapV2Pair) {
            uint256 balance = address(this).balance.div(2);
            if (balance > buyBackSellLimit) {
                buyBackTokens(buyBackSellLimit);
            } 
        }
    ...
    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee) external hasEnoughUsdToChangeStuff {
        require(sellTaxFee <= maxTaxFee, "Tax exceeds limits");
        require(sellLiquidityFee <= maxTaxFee, "Tax exceeds limits");
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee) external hasEnoughUsdToChangeStuff {
        require(buyTaxFee <= maxTaxFee, "Tax exceeds limits");
        require(buyLiquidityFee <= maxTaxFee, "Tax exceeds limits");
