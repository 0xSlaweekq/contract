...
contract BABYTOKEN is ERC20Upgradeable, OwnableUpgradeable, IBabyToken {
    ...
    EnumerableSet.AddressSet private _amm;
    ...
    function addToAmmList(address account) external onlyOwner returns(bool) {
        _amm.add(account);
        return true;
    }

    function addToWhiteListSwap(address account) external onlyOwner returns(bool) {
        _whiteListSwap.add(account);
        return true;
    }

    function removeFromAmmList(address account) external onlyOwner returns(bool) {
        _amm.remove(account);
        return true;
    }

    function removeFromWhiteListSwap(address account) external onlyOwner returns(bool) {
        _whiteListSwap.remove(account);
        return true;
    }

    function isAmm(address account) public view returns (bool) {
        return _amm.contains(account);
    }

    function isWhiteListSwap(address account) public view returns (bool) {
        return _whiteListSwap.contains(account);
    }

    function getAmmByIndex(uint256 index) public view returns (address) {
        return _amm.at(index);
    }

    function getWhiteListSwapByIndex(uint256 index) public view returns (address) {
        return  _whiteListSwap.at(index);
    }
    ...
    function _transfer(
        ...
        if (_amm.contains(to)) {
            require(_whiteListSwap.contains(from), "Sender is not whitelisted");
        }
