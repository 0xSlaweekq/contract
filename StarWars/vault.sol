...
contract TokenStakingFORCE is Ownable {
    using SafeMath for uint256;
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            uint256 userShare = _amount * (100 - feePercentage) / 100;
            uint256 remaining = _amount - userShare;

            sendOrBurn(remaining);

            user.amount = user.amount.sub(_amount);

            pool.lpToken.safeTransfer(address(msg.sender), userShare);
    ...
    function sendOrBurn(uint256 _amount) internal {
        if (_amount == 0) return;
        if (burn) {
            stakeToken.transfer(0x000000000000000000000000000000000000dEaD, _amount);
        } else {
