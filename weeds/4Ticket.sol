...
contract WeedsLabTickets is ERC165, IERC721, IERC721Metadata, Ownable {
  ...
  uint256[] private mintedTikets;

  // Token name
  string private _name = 'Weeds Lab Tickets Official';

  // Token symbol
  string private _symbol = 'wTicket';
  ...
  function getCurrentPrice() external view returns (uint256) {
    uint8 gen;
    if (_circulatingSupply <= 100000) gen = 0;
    else return 0;
  ...
  function getNumOfMintedTikets() public view returns (uint256) {
    return mintedTikets.length;
  }

  function getNFTMetadata(uint256 tokenID) external view virtual returns (uint8) {
    require(_revealed, 'Tokens were not yet revealed');
    require(_exists(tokenID), 'Token does not exist');
    return (_nftMetadata[tokenID].gen);
  }

  function getMintedTikets() external view returns (uint256[] memory) {
    return mintedTikets;
  }
  ...
  function mint(uint256 _amount) external payable {
    ...
    uint256 price;
    if (_circulatingSupply < 100000 && _circulatingSupply + _amount <= 100000) {
  ...
  function changeMintPrice(uint256 _ticketPrice) external onlyOwner {
  ...
  function _getCurrentPrice(uint8 gen) internal view returns (uint256) {
    ...
    else if (gen == 0) return ticketPrice;
    else revert();
  }
  ...
  function _mint(address to, uint256 tokenId) internal virtual {
    ...
    if (tokenId <= 100000) _nftMetadata[tokenId].gen = 0;

    mintedTikets.push(tokenId);
    _balances[to] += 1;
    _owners[tokenId] = to;
