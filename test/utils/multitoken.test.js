...
const revertMessages = {
  ...
  submitTributeProposalMaxGuildBankTokensReached:
    'cannot submit more tribute proposals for new tokens - guildbank is full',
  sponsorTributeProposalMaxGuildBankTokensReached:
  ...
  ([
    ...
    describe('collectTokens', () => {
      ...
      it('happy case - collect tokens', async () => {
        ...
        assert.equal(
          molochTokenAlphaBalance,
          proposal1.tributeOffered + tokensToCollect + deploymentConfig.PROPOSAL_DEPOSIT
        );
      ...
      it('require fail - must be member to collect', async () => {
        ...
        await moloch
          .collectTokens(tokenAlpha.address, { from: proposal2.applicant }) // attempt to collect from non-member
      ...
      it('require fail - token to collect not whitelisted', async () => {
        ...
        await moloch
          .collectTokens(tokenGamma.address, { from: summoner })
      ...
      it('require fail - guild bank balance for token to collect is zero', async () => {
        ...
        await moloch
          .collectTokens(tokenBeta.address, { from: summoner })
    ...
    describe('multi-token ragequit + withdraw', async () => {
      beforeEach(async () => {
        ...
        await verifyBalances({
          ...
          expectedMolochBalance: proposal1.tributeOffered.add(
        ...
        await verifyBalances({
          ...
          moloch: moloch.address,
          expectedMolochBalance: proposal1.tributeOffered.add(
            new BN(deploymentConfig.PROPOSAL_DEPOSIT * 2)
          ), // tribute now in bank,
          applicant: proposal1.applicant,
          expectedApplicantBalance: 0
        });
        ...
        await verifyInternalBalances({
          ...
          userBalances: {
            ...
            [proposal1.applicant]: 0,
            [summoner]:
              (deploymentConfig.PROPOSAL_DEPOSIT - deploymentConfig.PROCESSING_REWARD) * 2,
      ...
      describe('happy path - ', () => {
        ...
        it('member shares reduced', async () => {
          ...
          assert.equal(
            +totalShares,
            summonerShares + proposal1.sharesRequested + proposal2.sharesRequested - sharesToQuit
          );
        ...
        describe('withdraw balances', () => {
          ...
          beforeEach(async () => {
            ...
            applicantTokenBalances = [
              _1e18.mul(sharesToQuit).div(initialShares.add(initialLoot)),
              _1e18.mul(sharesToQuit).div(initialShares.add(initialLoot))
            ];
          });
          ...
          it('set max = true', async () => {
            ...
            await verifyBalances({
              ...
              expectedMolochBalance: proposal1.tributeOffered
                .add(new BN(deploymentConfig.PROPOSAL_DEPOSIT * 2))
                .sub(applicantTokenBalances[0]),
              applicant: proposal1.applicant,
          ...
          it('full withdrawal (without max)', async () => {
            await moloch.withdrawBalances(tokens, applicantTokenBalances, false, {
              from: proposal1.applicant
            });
          ...
          it('partial withdrawal', async () => {
            ...
            await moloch.withdrawBalances(tokens, withdrawAmounts, false, {
              from: proposal1.applicant
            });
          ...
          it('require fail - insufficient balance (1st token)', async () => {
            ...
            await moloch
              .withdrawBalances(tokens, applicantTokenBalances, false, {
                from: proposal1.applicant
              })
    ...
    describe('token count limit - add tokens during operation', async () => {
      ...
      it('require fail - sponsor another whitelist proposal when maximum reached', async () => {
        await moloch
          .sponsorProposal(thirdProposalIndex, { from: summoner })
      ...
      it('require fail - submit another whitelist proposal when maximum reached', async () => {
        await moloch
          .submitWhitelistProposal(tokenEpsilon.address, 'whitelist epsilon!', { from: summoner })
          .should.be.rejectedWith(
            revertMessages.submitWhitelistProposalMaximumNumberOfTokensReached
          );
      });
    });
    ...
    describe('guild bank token limit', () => {
      ...
      it('sponsoring a previously submitted tribute proposal fails after max guild bank tokens is reached', async () => {
        ...
        await moloch
          .sponsorProposal(initialProposalCount + 1, { from: summoner })
      ...
      it.only('can still ragequit and withdraw', async function () {
        ...
        await verifyInternalBalances({
          ...
          userBalances: {
            ...
            [summoner]:
              (deploymentConfig.PROPOSAL_DEPOSIT - deploymentConfig.PROCESSING_REWARD) *
              +MAX_TOKEN_GUILDBANK_COUNT.add(MAX_TOKEN_WHITELIST_COUNT).sub(_1).toString(),
            [processor]:
              deploymentConfig.PROCESSING_REWARD *
              +MAX_TOKEN_GUILDBANK_COUNT.add(MAX_TOKEN_WHITELIST_COUNT).sub(_1).toString()
          }
