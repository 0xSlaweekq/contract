...
contract Staking is Ownable, IERC721Receiver, Pausable {
  ...
  IGalaxyWars private nftContract;
  ...
  uint256 public stealPrice = 5 * 10 ** 16;
  ...
  constructor(address _token, address _nftContract, address[] memory _wallets, uint256[] memory _percentages) {
    nftContract = IGalaxyWars(_nftContract);
    token = IERC20(_token);
    uint256 total;
  ...
  function changeStealPriceAnHour(uint256 newPrice, uint256 startTime) external onlyOwner {
    stealchangeNewPrice = newPrice;
    if (startTime == 0) stealChangeStartTime = block.timestamp;
    else stealChangeStartTime = startTime;
  }
    ...
    if (farmStartDate != 0 && farmStartDate <= block.timestamp) {
      ...
      if (nft.nftType == 0) {
        ...
        if (stakedOfficers.length > 0) {
          uint256 _probability = uint256(keccak256(abi.encodePacked(blockhash(block.number), tx.origin, block.timestamp))) % 100000;
