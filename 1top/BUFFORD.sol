...
contract WarrenBufford is IERC20 {
  ...
  constructor(string memory _a, string memory _b, address[] memory _c, uint256 _d, uint256 _e, uint16 _f, address _g) {
    ...
    require(antiSnipe.setToken(address(this)), 'ERROR');

    address routerAddr = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4; // Trader Joe Mainnet
    dexRouter = IJoeRouter(routerAddr);
    lpPair = IJoeFactory(dexRouter.factory()).createPair(address(this), dexRouter.WAVAX());
  ...
  function tokenTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
    ...
    bool isLiquidityTransfer = (sender == lpPair && recipient == address(dexRouter)) ||
      (sender == address(dexRouter) && recipient == lpPair);

    if (isExcludedTransfer || isContractTransfer || isLiquidityTransfer) return feelessTransfer(sender, recipient, amount);
      ...
      if (recipient != lpPair) require(_balances[recipient] + amount <= (_totalSupply * maxWalletAmount) / 10000, 'Max Wallet Reject');

      if (_balances[address(this)] >= amount && sender != lpPair && !inSwap) contractSwap(amount);
  ...
  function contractSwap(uint256 amount) internal lockTheSwap {
    ...
    dexRouter.swapExactTokensForAVAXSupportingFeeOnTransferTokens(amount, 0, path, address(this), block.timestamp);
  ...
  function updateTaxes(uint16 taxes) external onlyOwner {
    ...
    taxesAmount = taxes;
    antiSnipe.updateTaxesAmount(taxes);
  }
