...
contract('Moloch', ([creator, summoner, applicant1, applicant2, processor, delegateKey, nonMemberAccount, ...otherAccounts]) => {
  ...
  describe('collectTokens', () => {
    ...
    it('require fail - token to collect not whitelisted', async () => {
      ...
      await moloch.collectTokens(tokenGamma.address, { from: summoner }).should.be.rejectedWith(revertMessages.collectTokensMustBeWhitelisted);
    ...
    it('require fail - guild bank balance for token to collect is zero', async () => {
      ...
      await moloch.collectTokens(tokenBeta.address, { from: summoner }).should.be.rejectedWith(revertMessages.collectTokensMustHaveBalance);
    ...
    describe('happy path - ', () => {
      ...
      describe('withdraw balances', () => {
        ...
        it('require fail - insufficient balance (1st token)', async () => {
          ...
          await moloch
            .withdrawBalances(tokens, applicantTokenBalances, false, { from: proposal1.applicant })
  ...
  describe('RAGEQUIT AND WITHDRAW TOKENS AT MAXIMUM TOKEN LIMITS', () => {
    ...
    beforeEach(async function () {
      ...
      for (let i = 1; i < tokens.length; i++) {
        ...
        await moloch.submitWhitelistProposal(token.address, 'whitelist this token!', { from: proposal1.applicant });
